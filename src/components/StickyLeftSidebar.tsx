...
export function StickyLeftSidebar({
  ...
  return (
    <div className="fixed left-0 top-0 h-screen w-72 bg-slate-800/95 backdrop-blur-sm border-r border-slate-700/50 z-20 overflow-y-auto">
      <div className="p-4 space-y-6">
        ...
        <div className="bg-slate-700/50 rounded-lg p-4 border border-slate-600">
          ...
          <div className={`text-xl font-bold flex items-center space-x-2 mb-2 ${getTotalProfitByFilter(profitFilter) >= 0 ? 'text-green-400' : 'text-red-400'}`}>
            <span>{getTotalProfitByFilter(profitFilter).toFixed(totalProfitCurrency === 'chaos' ? 2 : 3)}</span>
            <img src={CURRENCY_IMAGES[totalProfitCurrency]} alt={`${totalProfitCurrency} Orb`} className="w-5 h-5" />
          </div>
          <div className="text-sm text-slate-400 flex items-center space-x-1">
            ...
            <img src={CURRENCY_IMAGES[totalProfitCurrency === 'chaos' ? 'divine' : 'chaos']} alt={`${totalProfitCurrency === 'chaos' ? 'divine' : 'chaos'} Orb`} className="w-4 h-4" />
          </div>
        </div>
      </div>
    </div>
